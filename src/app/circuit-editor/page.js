...
export default function CircuitEditor() {
  ...
  const saveCurrentStateToHistory = useCallback(() => {
    ...
    if (historyIndex >= 0) {
      ...
      const nodesEqual = JSON.stringify(lastState.nodes) === JSON.stringify(currentState.nodes);
      const edgesEqual = JSON.stringify(lastState.edges) === JSON.stringify(currentState.edges);
  ...
  const handleNodesChange = useCallback((changes) => {
    ...
    const hasPositionChanges = changes.some(
      change => change.type === 'position' && (change.position?.x !== undefined || change.position?.y !== undefined)
    );
  ...
  const onDrop = useCallback(
    (event) => {
      ...
      const componentData = event.dataTransfer.getData('application/circuit-component');
  ...
  const handleDelete = useCallback(() => {
    if (selectedElements.nodes.length > 0 || selectedElements.edges.length > 0) {
      ...
      if (contextMenu.elementType === 'node') {
        setNodes(nodes => nodes.filter(node => node.id !== contextMenu.elementId));
      } else if (contextMenu.elementType === 'edge') {
        setEdges(edges => edges.filter(edge => edge.id !== contextMenu.elementId));
      }
      
      setContextMenu({ ...contextMenu, show: false });
    }
  }, [selectedElements, setNodes, setEdges, saveCurrentStateToHistory, contextMenu]);
    ...
    if (selectedElements.nodes.length > 0) {
      setClipboard({
        ...
        edges: selectedElements.edges.filter(edge => 
          selectedElements.nodes.some(node => node.id === edge.source) && 
          selectedElements.nodes.some(node => node.id === edge.target)
        )
      });
    } else if (contextMenu.show && contextMenu.elementType === 'node') {
      const nodeToCopy = nodes.find(node => node.id === contextMenu.elementId);
      if (nodeToCopy) {
        setClipboard({
          nodes: [nodeToCopy],
          edges: []
  ...
  }, [clipboard, reactFlowInstance, setNodes, setEdges, saveCurrentStateToHistory]);
  ...
  const handleExport = useCallback((format = 'png') => {
    if (format === 'png' && reactFlowInstance) {
      ...
      const exportFileDefaultName = `circuit-${new Date().toISOString().slice(0, 10)}.json`;
      ...
      setNodes(nds => 
        nds.map(node => {
          if (node.id === contextMenu.elementId) {
            ...
            const highestZIndex = Math.max(0, ...nds.map(n => n.zIndex || 0));
      ...
      setNodes(nds => 
        nds.map(node => {
          if (node.id === contextMenu.elementId) {
            ...
            const lowestZIndex = Math.min(0, ...nds.map(n => n.zIndex || 0));
  ...
  const rotateNode = useCallback((degrees) => {
    saveCurrentStateToHistory();
        ...
        nds.map(node => {
          if (node.id === contextMenu.elementId) {
            ...
            const currentRotation = node.style?.transform
              ? parseInt(node.style.transform.replace('rotate(', '').replace('deg)', ''), 10)
              : 0;
  ...
  return (
    <div className={styles.circuitEditorContainer}>
      ...
      <div className={styles.editorContent}>
        <CircuitComponentsPanel onComponentSelect={handleComponentSelect} />
            ...
            >
              ...
              <Panel position="top-right" className={styles.panel}>
                <div className={styles.panelContent}>
                  ...
                  <button onClick={handleCopy} disabled={selectedElements.nodes.length === 0} title="Copy (Ctrl+C)">
                    Copy
                  </button>
                  <button onClick={handlePaste} disabled={clipboard.nodes.length === 0} title="Paste (Ctrl+V)">
                    Paste
                  </button>
                  <button onClick={handleDelete} disabled={selectedElements.nodes.length === 0 && selectedElements.edges.length === 0} title="Delete (Delete)">
                    Delete
                  </button>
                  <button onClick={handleClear} title="Clear Canvas">Clear</button>
                  <button onClick={handleSave} title="Save">Save</button>
                  <button onClick={handleLoad} title="Load">Load</button>
                  <button onClick={() => handleExport('png')} title="Export as PNG">Export PNG</button>
